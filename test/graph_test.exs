defmodule ExGraphTest do
  ...
  describe "get_paths!/3; single vertex graph:" do
    setup do
      g = ExGraph.new()
      ExGraph.add_vertex(g, 1)
      {:ok, %{g: g}}
    end

    test "start and goal are the same", %{g: g} do
      assert {:ok, [[1]]} = ExGraph.get_paths!(g, 1, 1)
    end

    test "start and goal not in the graph", %{g: g} do
      assert {:ok, []} = ExGraph.get_paths!(g, 3, 4)
    end
  end
  ...
  describe "get_paths!/3; 3 vertex graph, tree structure:" do
    setup do
      g = ExGraph.new()
      ExGraph.add_vertices(g, [1,2,3])
      ExGraph.add_edges(g, [{1,2}, {2,3}])
      {:ok, %{g: g}}
    end

    test "start and goal connected by one edge", %{g: g} do
      assert {:ok, [[1,2]]} = ExGraph.get_paths!(g, 1, 2)
    end

    test "start and goal not connected", %{g: g} do
      assert {:ok, []} = ExGraph.get_paths!(g, 2, 1)
    end
  end
  ...
  describe "get_paths!/3; 3 vertex graph, cyclic structure:" do
    setup do
      g = ExGraph.new()
      ExGraph.add_vertices(g, [1,2,3])
      ExGraph.add_edges(g, [{1,2}, {1,3}, {3, 2}])
      {:ok, %{g: g}}
    end

    test "start and goal connected by 2 paths", %{g: g} do
      assert {:ok, [[1,2], [1,3,2]]} = ExGraph.get_paths!(g, 1, 2)
    end
  end
  ...
  describe "get_paths!/3; 5 vertices, cyclic: " do
    setup do
      g = ExGraph.new()
      ExGraph.add_vertices(g, [1,2,3,4,5])
      ExGraph.add_edges(g, [{1,2}, {2,1}, {1,3}, {3,1}, {1,4}, {4,3}, {3,5}])
      {:ok, %{g: g}}
    end

    test "start and goal could enter a cycle", %{g: g} do
      assert {:ok, [[1,2]]} = ExGraph.get_paths!(g, 1, 2)
      assert {:ok, [[1,3,5], [1,4,3,5]]} = ExGraph.get_paths!(g, 1, 5)
    end
  end
end
